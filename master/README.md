...
In order to work with image build from the latest [Nominatim](https://github.com/osm-search/Nominatim) master use this.

Just check the `docker-compose.yml` template which you use as a starting point and adapt to your needs.
Use this template to set the environment variables, mounts, etc. as needed.
Then just start the container with:
```
docker-compose up
```

You can connect to container image by calling:
```
